...
function Summary({ enabledNext }) {
    ...
    const GenerateSummaryFromAI = async () => {
        ...
        try {
            ...
            setAiGenerateSummaryList(JSON.parse(cleanJson));
        } catch (error) {
    ...
    const onSave = (e) => {
        ...
        const cleanResumeId = params?.resumeId?.split(':')[0];

        GlobalApi.UpdateResumeDetail(cleanResumeId, data).then(resp => {
            console.log(resp);
            enabledNext(true);
            setLoading(false);
            ...
            {aiGeneratedSummaryList && <div className='my-5'>
                ...
                {aiGeneratedSummaryList?.map((item, index) => (
                    <div key={index}
                        ...
                        className='p-5 shadow-lg my-4 rounded-lg cursor-pointer hover:shadow-xl hover:scale-105 transition-all'>
